...
import string
#确定结果sheet名
result_sheetname='LCD分套装点位推送及回传数据分析'+'XXX'+'销售用'
impt_xlsx_resource = pd.read_excel(outputdir + 'screen_monitor_0403.xlsx', sheet_name='媒体城市')
impt_xlsx_resource.replace('不分套装', '合计', inplace=True)
sheet4_dataframe = impt_xlsx_resource[(impt_xlsx_resource.媒体 == 'lcd') & (impt_xlsx_resource.套装 != '无套装')].pivot_table(
    index=['城市', '套装'],
    values=['可售安装总数', '推送成功总数', '分钟统计推送成功且最近一天>1h比率'])
...
merge_result.to_excel(outputdir + 'result_test.xlsx', sheet_name=result_sheetname)

##修改样式,遍历A-Z列
workbook = load_workbook(outputdir + 'result_test.xlsx')
...
workbook.write_only(filename=outputdir + 'result_test.xlsx')
