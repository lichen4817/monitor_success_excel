...
data_normal = {
    '城市': ['全国', '上海', '北京', '成都', '杭州', '深圳', '广州', '重庆', '天津', '武汉', '昆明', '大连', '南京', '青岛', '合肥', '济南', '长沙', '石家庄',
           '长春', '哈尔滨', '沈阳', '厦门', '苏州', '贵阳', '西安', '郑州', '福州', '无锡', '东莞', '太原', '温州', '海口', '烟台', '宁波', '泉州', '兰州',
           '佛山', '中山', '珠海', '柳州', '洛阳', '晋江', '绵阳', '廊坊', '漳州', '西宁', '昆山', '常德', '宜昌', '桂林', '惠州', '芜湖', '鄂尔多斯', '石狮',
           '江阴', '汕头', '沧州', '常熟', '西双版纳', '三明', '南通', '威海', '张家港', '台州', '泸州', '德阳', '泰州', '临沂', '滁州', '蒙自', '襄阳',
           '衡水', '遵义', '惠安', '绍兴', '义乌', '宜宾', '嘉兴', '安庆', '曲靖', '楚雄', '湖州', '瑞安', '南安', '平湖', '济宁', '玉溪', '湘潭', '泰安',
           '三亚', '清镇', '大理', '凯里', '丽江', '九江', '扬州', '安宁', '北碚', '仁怀', '邢台', '个旧', '唐山', '安顺', '咸阳', '都江堰', '新郑', '荥阳',
...
merge_result['全设备推送成功率'] = merge_result['全设备推送成功率'].apply(lambda x: format(x, '.2%'))
merge_result['智能屏推送成功率'] = merge_result['智能屏推送成功率'].apply(lambda x: format(x, '.2%'))
merge_result['小时统计推送成功且七天平均>20h比率'] = merge_result['小时统计推送成功且七天平均>20h比率'].apply(lambda x: format(x, '.2%'))
...
writer.book = book
merge_result.to_excel(excel_writer=writer,sheet_name=result_sheetname)
writer.save()
writer.close()
